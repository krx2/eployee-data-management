...
class EmployeeIntegrationTest {
    ...
    void createThenGetReturnsEmployeeWithoutPlaintextSsn() throws Exception {
        ...
        String createResponseBody = mockMvc.perform(authorized(post("/employees"))
    ...
    void getMissingEmployeeReturns404() throws Exception {
        mockMvc.perform(authorized(get("/employees/{id}", UUID.randomUUID())))
    ...
    void deleteThenGetReturns404() throws Exception {
        String createResponseBody = createEmployee("444-44-4444");
        UUID id = UUID.fromString(objectMapper.readTree(createResponseBody).get("id").asText());
    ...
    void listIsPaginatedAcrossMultiplePages() throws Exception {
        ...
        mockMvc.perform(authorized(get("/employees")).param("page", "0").param("size", "2"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content.length()").value(2))
                .andExpect(jsonPath("$.totalElements").value(3))
                .andExpect(jsonPath("$.totalPages").value(2));

        mockMvc.perform(authorized(get("/employees")).param("page", "1").param("size", "2"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.content.length()").value(1))
                .andExpect(jsonPath("$.totalElements").value(3));
    }

    private String createEmployee(String ssn) throws Exception {
        EmployeeCreateRequest request = new EmployeeCreateRequest(
                "Jan", "Kowalski", LocalDate.of(1990, 1, 1), Gender.MALE, ssn);

        return mockMvc.perform(authorized(post("/employees"))
                        .contentType("application/json")
                        .content(objectMapper.writeValueAsString(request)))
                .andExpect(status().isCreated())
                .andReturn().getResponse().getContentAsString();
